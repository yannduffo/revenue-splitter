...
Plan d'action (option C)
  ...
  2. Schéma Postgres :
     - splitters (avec les membres et les parts, qui sont déjà dans l'événement) ;
     - incoming_transfers et claims, avec comme clé txHash-logIndex, plus block, logIndex et timestamp ;
     - cursor (bloc et hash).
  3. Boucle du worker, un tick toutes les 12 s :
     - (a) Lire les nouveaux SplitterCreated.
     - (b) Backfill des nouveaux splitters : Transfer et Claimed depuis leur createdAtBlock jusqu'au curseur global. C'est le
     - (c) Lancer la requête globale sur [curseur+1, tête−N].
     - Écrire les lignes et le curseur dans une seule transaction, avec ON CONFLICT DO NOTHING.
  4. Reorgs. Rester quelques blocs derrière la tête (N ≈ 5 à 12) et comparer le hash stocké du dernier bloc. En cas de divergence, supprimer les lignes au-delà de la
     divergence et reculer le curseur. Ça fait environ 30 lignes. N'attends pas le bloc finalized : avec environ 15 min de retard, la démo « j'envoie un token et il
     apparaît » ne marcherait plus. Les soldes, eux, restent en direct puisque ce sont des appels de vue.
     faudra peut-être découper en lots, par exemple de 500 adresses). Je ne connais pas la limite d'Infura, c'est à mesurer.
  2. Schéma Postgres :
     - splitters (avec les membres et les parts, qui sont déjà dans l'événement) ;
     - incoming_transfers et claims, avec comme clé txHash-logIndex, plus block, logIndex et timestamp ;
     - cursor (bloc et hash).
  3. Boucle du worker, un tick toutes les 12 s :
     - (a) Lire les nouveaux SplitterCreated.
     - (b) Backfill des nouveaux splitters : Transfer et Claimed depuis leur createdAtBlock jusqu'au curseur global. C'est le point subtil.
     - (c) Lancer la requête globale sur [curseur+1, tête−N].
     - Écrire les lignes et le curseur dans une seule transaction, avec ON CONFLICT DO NOTHING.
